...
if __name__=='__main__':
    ...
    df = process_data()
    df = df[['Numero episodio', 'EJEII Transtorno Personalidad y Comportamiento L', 'EJEII Transtorno Personalidad y Comportamiento']]

    # join both columns into one
    df.loc[:, 'EJEII'] = df['EJEII Transtorno Personalidad y Comportamiento L'].fillna('') + df['EJEII Transtorno Personalidad y Comportamiento'].fillna('')

    # lemmatize texts and get corpus 
    df['EJEII_lemmas'] = df['EJEII'].apply(lemmatize_and_preprocess)
    ...
    custom_stopwords = ['personalidad', 'transtorno', 'especificar', 'trastorno', 'insestabilidad', 'emocional', 
                        'histriónico', 'rasgo', 'cluster', 'rasgos', 'b', 'valorar', 'caracterial', 'c', 'estudio', 
                        'eje', 'ii', 'personalidad', 'h', 'impresiona', 'impresionar', 'dx', 'plano', 'especificadorasgo',
                        'leve', 'principal', 'específico', 'transtornos', 'tipo', 'personalidadotro', 'inestabilidad', 'ci',
                        'personalidadtrastorno', 'tr'] 
    ...
    topicModel.save('./models/EJEII.gensim')
    # modelo_lda = models.LdaModel.load("modelo_lda.gensim")
    vis = pyLDAvis.gensim.prepare(topicModel, corpus_bow, D)
    pyLDAvis.save_html(vis, './plots/lda_visualization_EJEII.html')
     
    labels_df = pd.DataFrame({'Topic': [i for i in range(0, 16)], 'Label': []})

    # add topic to the dataframe 
    df["ejeII"] = get_topic(topicModel, labels_df, df['EJEII_lemmas'])
    df = df[['Numero episodio', 'ejeII']]
    df.to_csv('./data/processed/ejeII.csv')