...
class PatientDataset(Dataset):
    ...
    def process_one_patient_appointments(df):
        ...
        return {
            "feat": torch.tensor(
                df.values, dtype=int
            ),  # list(map(partial(torch.tensor, dtype=int), list(df.values))),
            "iat": torch.log(torch.tensor(iat, dtype=int))
            if iat.size != 0
...
def custom_collate(batch):
    ...
    packed_histU = pad_pack_2d(histU)
    packed_histN = pad_pack_2d(histN)
...
def get_model_input():
    ...
    dataloader = DataLoader(
        data, batch_size=32, collate_fn=custom_collate, shuffle=True
    )
...
if __name__ == "__main__":
    ...
    # Initialize your PoissonProcessModel
    model = NHP(3, 24, 1)
